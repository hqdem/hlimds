...
bool checkMiterCorrectness(const std::shared_ptr<model::SubnetBuilder> &miter) {
...
TEST(MiterTest, Random) {
  ...
  for (size_t i = 0; i < nSubnet; ++i) {
    const auto subnetBuilder = makeBuilderRandomMatrix(nIn, nOut, nCell,
                                                       minArity, maxArity, i);
    auto miter1 = std::make_shared<model::SubnetBuilder>();
    ...
    const auto subnetID1 = makeSubnetRandomMatrix(nIn, nOut, nCell, minArity,
                                                  maxArity, i);
    auto miter2 = std::make_shared<model::SubnetBuilder>();
