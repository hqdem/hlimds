...
Gate::Id XagMapper::mapIn(GNet &newNet) const {
  return newNet.addIn();
}

Gate::Id XagMapper::mapOut(const Gate::SignalList &newInputs,
  ...
  if (n0 > 0 || n1 > 0) {
    ...
    return newNet.addOut(Gate::Signal::always(valId));
  }
...
Gate::Id XagMapper::mapVal(bool value, GNet &newNet) const {
    if (value) {
        return newNet.addOne();
    } else {
        return newNet.addZero();
    }
}
