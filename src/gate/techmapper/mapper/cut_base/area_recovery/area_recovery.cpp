...
double
AreaRecovery::calculAreaFlow(Cut &cut, std::vector<double> &representAreaFlow,
                             eda::gate::model::Array<Subnet::Entry> &entries,
                             float minArea) {
  double areaFlow = minArea;
  ...
  for (uint64_t coneEntryIndex = cut.entryIdxs.size();
    ...
    for (Subnet::Link &inLink : coneCellInOrig.link) {
      if (cut.entryIdxs.find(inLink.idx) != cut.entryIdxs.end()) {
        ...
        areaFlow += representAreaFlow[inLink.idx] / inCell.refcount;
      }
    }
  }
  return areaFlow;
...
double
AreaRecovery::calculDepth(std::vector<double> &depth,
                          eda::gate::model::Array<Subnet::Entry> &entries,
...
void AreaRecovery::findBest() {
  ...
  for (uint64_t entryIndex = 0; entryIndex < std::size(entries); entryIndex++) {
    ...
    for (Cut &cut : cutsList) {
      ...
      double areaFlow =
          calculAreaFlow(cut, representAreaFlow, entries, minCellArea);
      double maxDepth = calculDepth(depth, entries, entryIndex, cut);

      if (representAreaFlow[entryIndex] == 0 ||
          areaFlow < representAreaFlow[entryIndex] ||
          (areaFlow == representAreaFlow[entryIndex] &&
           maxDepth < representDepth[entryIndex])) {

        representAreaFlow[entryIndex] = areaFlow;
