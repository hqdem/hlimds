...
static void checkSimpleCases(const AigMapper *premapper) {
...
static void checkConstCases(const AigMapper *premapper) {
  SatChecker &checker = SatChecker::get();
  Builder builder1;
  builder1.addInput();
...
static void checkMAJ(const AigMapper *premapper) {
  const size_t nIn = 5;
  Builder builder;
  LinkList links;
...
static void checkAND(const AigMapper *premapper) {
  SatChecker &checker = SatChecker::get();
  ...
  std::cout << Subnet::get(transformedAndSub).size() << std::endl;
  std::cout << Subnet::get(transformedAndTree).size() << std::endl;
...
static void checkOR(const AigMapper *premapper) {
  SatChecker &checker = SatChecker::get();
  ...
  std::cout << Subnet::get(transformedOrSub).size() << std::endl;
  std::cout << Subnet::get(transformedOrTree).size() << std::endl;
...
static void checkXOR(const AigMapper *premapper) {
  SatChecker &checker = SatChecker::get();
  ...
  std::cout << Subnet::get(transformedXorSub).size() << std::endl;
  std::cout << Subnet::get(transformedXorTree).size() << std::endl;
...
static void checkRandomSubnet(const AigMapper *premapper) {
