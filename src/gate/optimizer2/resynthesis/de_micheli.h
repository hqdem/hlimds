...
private:
  ...
  uint64_t heuristicArg1(const TruthTable &divisor,
                         const TruthTable &care) const;

  uint64_t heuristicArg2(const TruthTable &f1,
                         const TruthTable &notF1,
                         const TruthTable &divisor,
                         const TruthTable &care) const;                                    

  uint64_t heuristicArg3(const TruthTable &coveredOnes,
  ...
  void run(std::vector<MajNode> &topNodes, std::vector<MajNode> &tree,
  ...
  void updateTree(std::vector<MajNode> &tree,
                  ...
                  std::vector<Position> &toExpand,
                  Position position, const TruthTable &funcOld) const;

  void updateSibling(std::vector<MajNode> &tree,
                     const std::vector<TruthTable> &divisors,
                     std::vector<Position> &toExpand, Position pos, uint8_t idx,
                     const TruthTable &funcOld, const TruthTable &sibling0,
                     const TruthTable &sibling1) const;

  void updateNode(std::vector<MajNode> &tree, 
                  const std::vector<TruthTable> &divisors,
                  std::vector<Position> toExpand, uint64_t siblingPos,
                  const TruthTable &careOld, const TruthTable &careNew) const;
  ...
  SubnetID buildSubnet(const std::vector<MajNode> &tree,
                       const std::vector<TruthTable> &divisors,
                       bool isSimpliest = false) const;

  Link createLink(int64_t arg, uint32_t nVars, size_t treeSize,
