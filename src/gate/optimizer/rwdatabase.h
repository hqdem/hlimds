...
public:
  ...
  virtual void push(const TruthTable key, const BoundGNet &value) {
    if (contains(key)) {
      _storage[key.raw()].push_back(value);
    } else {
      _storage[key.raw()] = {value};
    }
  }

  virtual void push(const TruthTable key, const BoundGNetList &value) {
    if (contains(key)) {
      for (const auto &bnet : value) {
        _storage[key.raw()].push_back(bnet);
      }
    } else {
      _storage[key.raw()] = value;
    }
...
public:
  ...
  BoundGNetList getFromDB(const TruthTable key);
  ...
  // Check whether db contains key
  bool dbContains(const TruthTable key);

  // Push new bound gnet in DB
  void pushInDB(const TruthTable key, const BoundGNet &value);

  // Push bound gnet list in DB
  void pushInDB(const TruthTable key, const BoundGNetList &value);
