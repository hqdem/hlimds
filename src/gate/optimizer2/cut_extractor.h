...
public:
  ...
  using CutsEntries = std::vector<std::unordered_set<std::size_t>>;
  ...
  struct Cut {
    ...
    Cut(const unsigned long long sign,
      ...
      signature(sign), entryIdxs(entryIdxs) {};
  ...
  /**
   * @brief Get cuts (entries indexes) for cell with entryIdx index.
   */
  CutsEntries getCutsEntries(uint64_t entryIdx) const;

private:
  /**
   * @brief Find all cuts for cell with entryIdx index.
   */
  CutsList findCuts(const Subnet &subnet, const std::size_t entryIdx,
  ...
  Cut getOneElemCut(const Subnet &subnet, const std::size_t entryIdx) const;
