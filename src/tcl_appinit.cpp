...
static int readVerilog(
  ...
  if (!std::filesystem::exists(app.remaining().at(0))) {
    Tcl_SetObjResult(interp, Tcl_NewStringObj("File doesn't exists", -1));
    return TCL_ERROR;
  }
...
bool checkEquivalence() {
  ...
  return checker.areEquivalent(*designBuilder, prev_step, "original").equal();
}
...
static int help(
  ...
  if (!helpFile.is_open()) {
    Tcl_SetObjResult(interp, Tcl_NewStringObj("Unable to open help file", -1));
    return TCL_ERROR;
  }
...
static int verilogToFir(
  ...
  for (const auto &file: app.remaining()) {
    if (!std::filesystem::exists(app.remaining().at(0))) {
      std::cout << file;
      Tcl_SetObjResult(interp, Tcl_NewStringObj(" doesn't exists", -1));
      return TCL_ERROR;
    }
  }
...
int Utopia_TclInit(Tcl_Interp *interp) {
  ...
  Tcl_CreateCommand(interp, "techmap", tmap, nullptr, nullptr);
